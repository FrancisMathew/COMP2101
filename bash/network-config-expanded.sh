...
interfaceslist=$(ifconfig | grep -w -o '^[^ ][^ ]*:' | tr -d :)

for interface in $interfaceslist; do

    #  address and hostname for the interface being summarized
    ...
    #  network number for this interface and its name if it has one
